...
interface DataMappingModalProps {
    isOpen: boolean;
    onClose: () => void;
    // FIX: Changed onConfirm parameter type to DataRow[] to match the fully processed data.
    onConfirm: (mappedData: DataRow[]) => void;
...
export const DataMappingModal: React.FC<DataMappingModalProps> = ({ isOpen, onClose, onConfirm, data, headers }) => {
    ...
    useEffect(() => {
        ...
        for (const key in REQUIRED_FIELDS) {
            ...
            // FIX: Coerced fieldKey to string to prevent calling toLowerCase on a potential number.
            const likelyMatch = headers.find(h => h.toLowerCase().replace(/[\s_]/g, '') === String(fieldKey).toLowerCase().replace(/[\s_]/g, ''));
            if (likelyMatch) {
    ...
    const handleConfirm = () => {
        ...
        // FIX: Removed 'as any' as mappedData now correctly matches the expected DataRow[] type.
        onConfirm(mappedData as unknown as DataRow[]);
    };
